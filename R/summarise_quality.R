...
#' }
#' @export
summarise_quality <- function(folder_sequences = "input.folder", trim.cutoff = 0.01, secondary.peak.ratio = 0.33, processors = NULL) {
  ...
  abi.fnames <- list.files(folder_sequences, pattern = "\\.ab1$", full.names = T, recursive = T)
