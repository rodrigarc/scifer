...
if (is.null(dim(sf[["summaries"]]))) {
  print("No files were processed, knitting was stopped early. Please double check the folder location provided for `data_folder` with ab1 files.")
  knitr::knit_exit()
} else if (is.null(sf[["summaries"]][["raw.length"]]) | is.na(sf[["summaries"]][["raw.length"]])) {
  print("Columns inside sf_summaries are empty (NA or NULL). Please double check the folder location provided for `data_folder` with ab1 files.")
  knitr::knit_exit()
...
processed_index <- fcs_processing(folder_path = folder_path_fcs, compensation = TRUE, plate_wells = "96", probe1 = "Pre.F", probe2 = "Post.F", posvalue_probe1 = 600, posvalue_probe2 = 400)
